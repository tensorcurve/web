...
function tc_create_structure($samples=false,$set_home=false,$set_brand=false){
 ...
 'h100-pricing'=>array('H100 rental price per hour','<p>Which cloud publishes the cheapest NVIDIA H100? The table below lists every supplier we track that posts a public H100 price, normalised to US dollars per GPU-hour, with on-demand, spot and commitment-term rates side by side. It is collected automatically each day; the check date at the top is the last successful run.</p>','publish','page-tracker.php'),
 'h200-pricing'=>array('H200 rental price per hour','<p>Public NVIDIA H200 rental prices from the suppliers we track, normalised to US dollars per GPU-hour and refreshed daily. Fewer suppliers publish H200 term pricing than H100; missing cells are missing from the source, not estimated.</p>','publish','page-tracker.php'),
 'a100-pricing'=>array('A100 rental price per hour','<p>Public NVIDIA A100 80GB rental prices from the suppliers we track, normalised to US dollars per GPU-hour and refreshed daily. Missing cells are missing from the source, not estimated.</p>','publish','page-tracker.php'),
 ...
 foreach($defs as $slug=>$data){
  ...
  if($data[3]==='page-tracker.php'){foreach(tc_tracker_gpus() as $gk=>$gm){if($gm['slug']===$slug){update_post_meta($id,'_tc_gpu',$gk);wp_update_post(array('ID'=>$id,'post_excerpt'=>$gm['excerpt']));}}}
 }
